...
public partial class Employee_EmployeeEdit : HRMS.Common.BasePage
{
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            Page.Validate("EditGrp");
            if (!Page.IsValid)
            {
                lblMsg.CssClass = "text-danger";
                lblMsg.Text = "⚠ 请检查上方红色提示，完善必填项后再保存。";
                return;
            }
            ...
            {
                DateTime bdt;
                if (!DateTime.TryParse(txtBirthday.Text, out bdt))
                {
                    lblMsg.CssClass = "text-danger";
                    lblMsg.Text = "⚠ 出生日期格式错误！";
                    return;
                }
                bd = bdt;
            }
            DateTime hd;
            if (!DateTime.TryParse(txtHireDate.Text, out hd))
            {
                lblMsg.CssClass = "text-danger";
                lblMsg.Text = "⚠ 入职日期格式错误！";
                return;
            }
            ...
            var emp = new Employee
            {
                ...
                Gender = string.IsNullOrEmpty(rblGender.SelectedValue) ? "男" : rblGender.SelectedValue,
                Birthday = bd,
                IdCard = string.IsNullOrWhiteSpace(txtIdCard.Text) ? null : txtIdCard.Text.Trim(),
                HireDate = hd,
                ...
                Status = string.IsNullOrEmpty(ddlStatus.SelectedValue) ? "在职" : ddlStatus.SelectedValue
            };
